本章内容对应的[PPT1](../../PPT/第5章%20插值方法与计算实验-第一次课.pdf) , [PPT2](../../PPT/第5章%20插值方法与计算实验-第二次课-带计算实验.pdf)，[参考代码](../../Code/插值方法.cpp)
...
- 第 $3$ 类边界条件： $f(x)$ 是周期函数，满足条件 $S(x_0)=S(x_n),S^{'}(x_0^+)=S^{'}(x_n^-),S^{''}(x_0^+)=S^{''}(x_n^-)$
